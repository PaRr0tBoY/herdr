    ...
    let toast_hit_area = app
        ...
        .map(|toast| {
            toast_notification_rect(
                ...
                toast.position.unwrap_or(app.toast_config.hive.position),
            )
        })
        .unwrap_or_default();
...
mod tests {
    ...
    fn copy_feedback_offset_only_increases_when_toast_rect_overlaps() {
        ...
        let bottom_right_toast = toast_notification_rect(
            ...
            &toast,
            false,
            crate::config::ToastHivePosition::BottomRight,
        );
        assert_eq!(
    ...
    fn desktop_toast_hit_area_uses_full_frame_not_terminal_area() {
        ...
        app.selected = 0;
        app.mode = Mode::Terminal;
        app.toast_config.hive.position = crate::config::ToastHivePosition::TopLeft;
    ...
    fn temp_git_repo(branch: &str) -> std::path::PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("unix time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("hive-ui-test-{unique}"));
