...
pub fn stop_command_for(name: Option<&str>) -> String {
    match name {
        Some(name) => format!("hive session stop {name}"),
        None => "hive server stop".to_string(),
    }
}
...
mod tests {
    ...
    fn local_stop_command_uses_server_stop_for_default_session() {
        ...
        assert_eq!(local_stop_command(), "hive server stop");
    ...
    fn local_stop_command_uses_session_stop_for_named_session() {
        ...
        assert_eq!(local_stop_command(), "hive session stop work");
    ...
    fn restart_after_update_guidance_names_stop_and_attach_commands() {
        assert_eq!(
            ...
            "hive session stop work",
            Some("hive session attach work")
            ),
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `hive session stop work`, then run `hive session attach work` again."
        );
    }
