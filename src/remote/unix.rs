...
const REMOTE_BINARY_ENV_VAR: &str = "HIVE_REMOTE_BINARY";
...
fn remote_binary_matches(ssh: &RemoteSsh, remote_herdr: &RemoteHerdr) -> io::Result<bool> {
    ...
    Ok(version == format!("hive {}", current_version())
...
fn warn_if_remote_bin_not_on_path(ssh: &RemoteSsh) -> io::Result<()> {
    ...
    eprintln!(
        "hive: installed remote binary to ~/.local/bin/hive, but the remote shell does not resolve `hive` to that path"
    );
    Ok(())
}
...
fn remote_shell_resolves_managed_install(stdout: &str) -> bool {
    stdout
        ...
        .next()
        .map(str::trim)
        .is_some_and(|path| path.ends_with("/.local/bin/hive"))
}
...
fn remote_release_asset(asset_key: &str) -> io::Result<RemoteReleaseAsset> {
    if crate::build_info::is_preview() {
        let build_id = crate::build_info::build_id().ok_or_else(|| {
            io::Error::other("preview client has no build id; set HIVE_REMOTE_BINARY or install Herdr on the remote manually")
        })?;
...
fn reattach_command(
    ...
    let program = if program.is_empty() { "hive" } else { program };
                ...
                match listener.accept() {
                    Ok((stream, _addr)) => {
                        ...
                        if let Err(err) = bridge_connection(
                            ...
                            eprintln!("hive: remote bridge failed: {err}");
                        }
                    }
                    ...
                    Err(err) => {
                        eprintln!("hive: remote bridge listener failed: {err}");
                        break;
                    }
                }
            }
...
fn private_ssh_config_dir() -> io::Result<PathBuf> {
    ...
    for base in bases {
        for attempt in 0..100 {
            let dir = base.join(format!("hive-ssh-{}-{attempt}", std::process::id()));
...
fn run_client_process(
    ...
    let status = Command::new(exe)
        ...
        .env("HIVE_RENDER_ENCODING", "terminal-ansi")
...
fn local_forward_socket_path(target: &str, session_name: &str) -> PathBuf {
    ...
    let readable = tmpdir.join(format!(
        "hive-remote-{pid}-{target_clean}-{session_clean}.sock"
    ));
    ...
    let short_name = format!("hive-r-{pid}-{target_prefix}-{hash}.sock");
...
mod tests {
    ...
    fn bridge_socket_is_user_only() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "hive-bridge-permissions-test-{}.sock",
            std::process::id()
        ));
    ...
    fn install_source_description_uses_override_binary() {
        ...
        assert_eq!(
            ...
            "HIVE_REMOTE_BINARY (/tmp/herdr-aarch64)"
        );
    }
    ...
    fn local_forward_socket_path_uses_readable_name_when_it_fits() {
        ...
        let filename = path
            ...
            .to_string();
        assert!(
            filename.starts_with("hive-remote-"),
    ...
    fn install_source_cleanup_removes_temporary_directory() {
        let dir = std::env::temp_dir().join(format!(
            "hive-install-source-cleanup-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
