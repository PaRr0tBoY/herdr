...
pub(crate) const STARTUP_CWD_ENV_VAR: &str = "HIVE_STARTUP_CWD";
...
/// Checks whether a hive server is listening at a specific socket path.
...
fn validate_running_server_compatibility() -> io::Result<()> {
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a hive server is listening, but its status API is unavailable.\n\n{}\nIf that fails, stop the old server process manually.",
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `HIVE_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    ...
    let child = command.spawn().map_err(|err: io::Error| {
        io::Error::new(err.kind(), format!("failed to spawn hive server: {err}"))
    })?;
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        std::env::set_var("HIVE_CLIENT_SOCKET_PATH", "/tmp/inherited-client.sock");
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("HIVE_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var("HIVE_CLIENT_SOCKET_PATH");
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
    }
