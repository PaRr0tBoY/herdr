...
use crate::{
    ...
    config::{ToastClipboardPosition, ToastHivePosition},
    detect::AgentState,
};
...
pub(crate) fn toast_notification_rect(
    ...
    let x = match position {
        ToastHivePosition::TopLeft | ToastHivePosition::BottomLeft => area.x,
        ToastHivePosition::TopRight | ToastHivePosition::BottomRight => {
            area.x + area.width.saturating_sub(width)
        }
    };
    let warning_offset = u16::from(offset_for_warning);
    let y = match position {
        ToastHivePosition::TopLeft | ToastHivePosition::TopRight => {
            area.y + warning_offset.min(area.height)
        }
        ToastHivePosition::BottomLeft | ToastHivePosition::BottomRight => {
...
mod tests {
    use super::*;
    use crate::config::{ToastClipboardPosition, ToastHivePosition};
    ...
    fn toast_rect_uses_configured_corner() {
        ...
        let top_left = toast_notification_rect(area, &toast, false, ToastHivePosition::TopLeft);
        assert_eq!(top_left.x, area.x);
        assert_eq!(top_left.y, area.y);

        let top_right = toast_notification_rect(area, &toast, false, ToastHivePosition::TopRight);
        assert_eq!(top_right.x + top_right.width, area.x + area.width);
        assert_eq!(top_right.y, area.y);

        let bottom_left =
            toast_notification_rect(area, &toast, false, ToastHivePosition::BottomLeft);
        assert_eq!(bottom_left.x, area.x);
        assert_eq!(bottom_left.y + bottom_left.height, area.y + area.height);

        let bottom_right =
            toast_notification_rect(area, &toast, false, ToastHivePosition::BottomRight);
    ...
    fn toast_rect_uses_display_width_for_cjk_labels() {
        ...
        let rect = toast_notification_rect(area, &toast, false, ToastHivePosition::TopRight);
