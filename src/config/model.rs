...
pub struct ToastConfig {
    ...
    pub hive: HiveToastConfig,
    pub clipboard: ClipboardToastConfig,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize)]
#[serde(default)]
pub struct HiveToastConfig {
    pub position: ToastHivePosition,
}
...
pub struct KeysConfig {
    ...
    /// Local-client shortcut that send a clipboard image to a remote Hive session. Default: "ctrl+v".
...
pub struct WorktreesConfig {
    /// Root directory under which Hive creates <repo>/<branch-slug> checkouts.
...
pub struct UiConfig {
    ...
    /// Terminal width at or below which Hive uses the mobile single-column layout. Default: 64.
    pub mobile_width_threshold: u16,
    /// Capture mouse input for Hive's mouse UI. Default: true.
...
pub struct ExperimentalConfig {
    /// Allow launching hive inside an existing hive pane. Default: false.
...
impl Default for ToastConfig {
    fn default() -> Self {
        Self {
            ...
            delay_seconds: 1,
            hive: HiveToastConfig::default(),
            clipboard: ClipboardToastConfig::default(),
        }
    }
}

impl Default for HiveToastConfig {
    fn default() -> Self {
        Self {
            position: ToastHivePosition::BottomRight,
        }
    }
}
...
"#;
    ...
    fn worktrees_directory_defaults_and_parses() {
        let default_config = Config::default();
        assert_eq!(default_config.worktrees.directory, "~/.hive/worktrees");
...
"#;
        ...
        assert_eq!(config.ui.toast.hive.position, ToastHivePosition::TopLeft);
    ...
    fn toast_config_defaults_preserve_existing_behavior_with_delay() {
        ...
        assert_eq!(
            config.ui.toast.hive.position,
            ToastHivePosition::BottomRight
        );
        assert!(config.ui.toast.clipboard.enabled);
        assert_eq!(
            config.ui.toast.clipboard.position,
...
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.ui.toast.delivery, ToastDelivery::Hive);
    }
