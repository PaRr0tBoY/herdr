...
if ([string]::IsNullOrWhiteSpace($env:HIVE_SOCKET_PATH)) { exit 0 }
...
& hive pane report-agent-session $env:HIVE_PANE_ID --source hive:copilot --agent copilot --agent-session-id $sessionId --seq $seq 2>$null | Out-Null
