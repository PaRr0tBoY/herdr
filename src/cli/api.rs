...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: hive api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Hive API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `hive api schema --json` to print the full schema.\nUse `hive api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_schema_help() {
    eprintln!("usage: hive api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Hive API schema"));
        assert!(text.contains("Use `hive api schema --json`"));
        assert!(text.len() < 400);
    }
}
